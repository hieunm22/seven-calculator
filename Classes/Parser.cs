...
{
    public enum Mode { RAD, DEG, GRA };

    public class Parser
    {
        private ArrayList FunctionList = new ArrayList(new string[] { "abs", 
            "acos", 
            "asin", 
            "atan", 
            "ceil", 
        ...
        public bool Evaluate(string Expression)
        {
            try
            {
                ...
                //
                //                                                                        ____
                // -(5-10)^(-1)*(3+2*(cos(3*Pi)+(2+ln(rowIndex))^3)) -> -(5-10)^(-1)*(3+2*(cos(3*3,14)+(2+ln(exp(1)))^3))
                //                          --
                //
                regEx = new Regex("pi", RegexOptions.IgnoreCase);
                Expression = regEx.Replace(Expression, "3141592653589793238462643".Insert(1, new calc().decimalSym));
                //
                // ----------
                // - Step 4 -
                // ----------
                ...
                while (m.Success)
                {
                    if (m.Groups[3].Value.Length > 0)
                    {
                        Expression = Expression.Replace(m.Value, "{" + m.Groups[1].Value + 
                            this.Solve(m.Groups[2].Value) + "}" + m.Groups[3].Value);
                    }
                    else
                    {
        ...
        private string Solve(string Expression)
        {
            ...
            while (m.Success)
            {
                ...
                Expression = regEx.Replace(Expression, this.Fact(Convert.ToInt32(m.Groups[1].Value)).ToString(), 1);
                m = regEx.Match(Expression);
            }
            ...
            while (m.Success)
            {
                decimal n = Convert.ToDecimal(m.Groups[1].Value);
                // Value negative or not integer -> throw exception
                if ((n < 0) && (n != Math.Round(n))) throw new Exception("Value negative or not integer");
                Expression = regEx.Replace(Expression, Fact(Convert.ToInt32(m.Groups[1].Value)).ToString(), 1);
                m = regEx.Match(Expression);

                //double n = Convert.ToDouble(m.Groups[1].Value);
                //if ((n < 0) && (n != Math.Round(n))) throw new Exception(); // Value negative or not integer -> throw exception
                //Expression = regEx.Replace(Expression, Fact(Convert.ToInt64(m.Groups[1].Value)).ToString(), 1);
                //m = regEx.Match(Expression);
            }
            // Solve power.
            ...
            regEx = new Regex(@"(\d+,*\d*rowIndex[\+-]?\d+|\d+,*\d*)\^(-?\d+,*\d*[eE][\+-]?\d+|-?\d+,*\d*)"); // Search for patterns like 5^-1
            m = regEx.Match(Expression, 0);
            while (m.Success)
            {
        ...
        // Calculate Factorial.
        private double Fact(int inp_num)
        {
            //return inp_num == 0 ? 1.0 : Fact(inp_num - 1) * inp_num;
            if (inp_num == 0)
            {
                return 1.0;
            }
            else
            {
                if (inp_num < 170)
                {
                    return Fact(inp_num - 1) * inp_num;
                }
                else
                {
                    MessageBox.Show("The input number is to large to calculate", "Over flow exception", 
