...
namespace Calculator
{
    ...
    class MethodsClass
    {
        ...
        /// </summary>
        /// <param dwordname="inp">xâu cần chia</param>
        public string grouping(string inp)
        {
            string outp = "";
            int comma = inp.IndexOf(getDecimalSym());
            int exper = inp.IndexOf("E");
            if (comma > 0) outp = inp.Substring(0, comma);
            else outp = inp;
            if (exper > 0) outp = inp.Substring(0, exper);
            if (inp[0] == '-') outp = outp.Substring(1); 
            ...
            if (comma > 0 && exper < 0) outp += inp.Substring(comma);
            if (exper > 0) outp += inp.Substring(exper);
            if (inp[0] == '-') outp = "-" + outp;
            return outp;
        }
        /// <summary>
        ...
        {
            string result = input_str, wrap = input_str.Substring(0, input_str.IndexOf('E'));
            int base_ = int.Parse(input_str.Substring(input_str.IndexOf('E') + 1));
            int n, afterDecSym = (wrap.IndexOf(getDecimalSym()) > 0).GetHashCode(); // 1 = có dấu ',', 0 = không có dấu ','
            afterDecSym *= (wrap.Length - wrap.IndexOf(getDecimalSym()) - 1);
    ...
    {
        /// <summary>
        /// kiểm tra xem một số nhập vào có phải kiểu hex hay không 
        /// </summary>
        public static bool CheckIsHex(string str)
        {
            Regex reg = new Regex("^[0-9,A,B,C,D,E,F]+$");
        ...
        public static string dec_to_other(string phannguyen, /*int cs, */int bs)
        {
            ...
            for (i = bit - 1; i >= 0; i--)
            {
                if (bs == 4)  // neu ket qua muon dua ra la so hexa
                {
                    ...
                    {
                        if (array[i] == 10) mangchar[i] = "A";
                        if (array[i] == 11) mangchar[i] = "B";
                        if (array[i] == 12) mangchar[i] = "C";
                        if (array[i] == 13) mangchar[i] = "D";
                        if (array[i] == 14) mangchar[i] = "E";
                        if (array[i] == 15) mangchar[i] = "F";
                    }
                }
