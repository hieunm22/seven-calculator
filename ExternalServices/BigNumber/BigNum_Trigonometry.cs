...
namespace Calculator
{
    public partial class BigNumber
    {
        ...
        static void Sinx(BigNumber src, BigNumber dst)
        {
            ...
            BigNumber f_comma_x = new BigNumber();  // f'(x)
            ...
            do
            {
                ...
                Power(src, 2 * n + 1, pow, NUM_DEFAULT_PLACES);
                pow.SignNum = (sbyte)(sign * src.SignNum);
                Div(pow, lastFactorial, f_comma_x, NUM_DEFAULT_PLACES);   //f'(x)=(-1)^x / (2x+1)!
                //dst = dst + f'(x);
                Add(dst, f_comma_x, add);
                Round(f_comma_x, f_comma_x, NUM_DEFAULT_PLACES);
                Round(add, add, NUM_DEFAULT_PLACES);
                Copy(add, dst);
                n++;
        ...
        static void Cosx(BigNumber src, BigNumber dst)
        {
            ...
            if (Compare(src, -BN_PI.Round(NUM_DEFAULT_PLACES)) < 0)
            {
                BigNumber rs = new BigNumber();
                Add(src, BN_DoublePI.Round(NUM_DEFAULT_PLACES), rs);
                Copy(rs, src);
            }
            else if (Compare(src, BN_PI.Round(NUM_DEFAULT_PLACES)) > 0)
            {
                BigNumber rs = new BigNumber();
                Sub(src, BN_DoublePI, rs);
                Copy(rs, src);
            }
            Sub(BN_HalfOfPi.Round(NUM_DEFAULT_PLACES), src, param);   // param = pi/2 - src
            Sinx(param, dst); 
        }
        /// <summary>
        /// tìm tanx
        ...
        private BigNumber ArcSinx(BigNumber src)
        {
            ...
            var source = (1 - src * src).Sqrt(NUM_DEFAULT_PLACES);
            result = ArcSinx(source);
            result = BN_HalfOfPi - result;
            result.SignNum = src.SignNum;
            return result;
        }
        /// <summary>
        /// tìm arccosx
